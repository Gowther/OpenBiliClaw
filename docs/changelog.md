...
- Web 控制台新增夜间模式：默认按 UTC+8 的 18:00-05:59 自动切到夜间，顶部按钮可在自动 / 夜间 / 日间之间手动覆盖。
...
- Web 消息逻辑对齐浏览器插件：历史 pending 惊喜推荐只进入推荐页惊喜队列，实时 `delight.candidate` 才进入消息；关闭惊喜消息会回写 `/api/delight/sent`，避免刷新后反复出现。
