...
如果只想在本地网页里看推荐、画像和聊天，后端启动后也可以直接打开 `http://127.0.0.1:8420/app/`。这个 Web 控制台复用现有后端 API，不读取浏览器 Cookie，也不采集页面行为；自动 Cookie 同步和 B 站页面行为采集仍需要插件，或手动把 Cookie 写入配置。
...
- 🖥️ **本地 Web 控制台** — 后端自带 `/app` 页面，可在普通浏览器里查看推荐、画像、聊天和运行状态
...
│ (行为采集 · 推荐/画像/聊天 UI · Cookie 同步 · xhs 任务/初始化) │
└──────────────────────────┬──────────────────────────────────┘
                           │ REST API / WebSocket（插件含 Cookie 同步请求）
┌──────────────────────────▼──────────────────────────────────┐
│                      Agent 编排层                            │
│                (Skill 系统 · 对话管理)                       │
├──────────┬──────────┬───────────┬────────────────────────────┤
